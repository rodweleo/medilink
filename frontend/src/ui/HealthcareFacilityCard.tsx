    ...
    return <Card className="flex w-[650px] cursor-pointer" key={healthcareFacility.id}>
            ...
            <CardContent className="flex flex-col gap-5">
                ...
                <div className="flex items-center gap-5">
                    <p className="flex items-center gap-1"><GiPathDistance /> <span className="text-sm text-blue-500 italic">{distance?.distance.value > 1000 ? `${(distance?.distance.value/1000).toPrecision(2)}KM` : `${distance?.distance.value}M`}</span></p>
                    <p className="flex items-center gap-1"><RiMapPinTimeLine  /> <span className="text-sm text-blue-500 italic">{distance?.duration.value > 60 ? `${(distance?.duration.value / 60).toPrecision(1)}Min` : `1 Min`}</span></p>
                </div>
            </CardContent>
